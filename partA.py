...
class RoadSegmenter_CAMERA:
    ...
    def region_grow(self):
        ...
        cv2.floodFill(
            flooded, mask, seed_point, 125, loDiff=5, upDiff=5, flags=8
        )  # Perform flood fill to segment the road region
        self.region = mask[1:-1, 1:-1]  # Remove the border added by flood fill

        self.filled_region = cv2.morphologyEx(
            self.region, cv2.MORPH_CLOSE, np.ones((12, 12), np.uint8)
        )  # Close small holes in the filled region
    ...
    def colorize_lanes(self, alpha=0.2):
        road_mask = (
            self.filled_region.astype(np.uint8) * 255
        )  # Convert the filled region το 0/255 mask
        ...
        for y in range(height):  # Iterate through each column of the road mask
            x_coords = np.where(road_mask[y] == 255)[
                0
            ]  # Get the x-coordinates of the road pixels in the current row
            if len(x_coords) > 1:
        ...
        self.mid_x_vals = (
            mid_x_vals  # Store midline x-coordinates in the instance variable
        )
    ...
    def compute_motion_vector_pca(self):
        ...
        if len(self.obstacles) == 0:
            ...
            end_point = (
                ...
                int(center[1] + scale * direction[1]),
            )
            cv2.arrowedLine(
